...
(function (window, d3) {
    var Utility = function () {
        ...
        _createClass(Utility, [{
            key: "defineReadonly",
            // jshint ignore:line
            value: function defineReadonly(obj, key, value) {
    ...
    var Stacked = function () {
        ...
        _createClass(Stacked, [{
            key: "draw",
            value: function draw(updatedData) {
                var that = this;
                ...
                that.stack = d3.layout.stack().values(function (d) {
                    return d.values;
                }).x(function (d) {
                ...
                that.yScale = d3.scale.linear().range([height, 0]).domain([0, that.maxY]);
                that.xScale = d3.time.scale().range([0, width]).domain(d3.extent(data[0].values, function (d) {
                    return new Date(Date.parse(d.date));
                })).nice(4);
                that.svg = d3.select("body").append(selector).attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
                that.colors = options.colors || d3.scale.category10();
                that.layers = that.svg.selectAll('g.layer').data(stacked, function (d) {
                    return d.name;
                }).enter().append('g').attr('class', 'layer').attr('fill', function (d) {
                    return that.colors(d.name);
                });
                that.layers.selectAll('rect').data(function (d) {
                    return d.values;
                }).enter().append('rect').attr('x', function (d) {
                    return that.xScale(new Date(Date.parse(d.date)));
                }).attr('width', width / 3).attr('y', function (d) {
            ...
            key: "on",
            value: function on(prop, event, callback) {
                var that = this;
    ...
    var DrawIt = function DrawIt(options) {
